                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('assetdatas.update', $asset->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control @error('industry_id') is-invalid @enderror" name="industry_id" id="industry_select" required>
                                            ...
                                            @foreach ($industries as $industry)
                                                <option value="{{ $industry->id }}" {{ old('industry_id', $asset->industry_id) == $industry->id ? 'selected' : '' }}>
                                                    {{ $industry->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6" id="other_industry_div" style="{{ old('industry_id', $asset->industry_id) != 4 ? 'display: none;' : '' }}">
                                    <div class="mb-3">
                                        <label class="form-label">Other Industry</label>
                                        <input type="text" class="form-control" name="other_industry" id="other_industry_input" value="{{ old('other_industry', $asset->other_industry) }}">
                                    </div>
                                </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control @error('utilization_id') is-invalid @enderror" name="utilization_id" required>
                                            ...
                                            @foreach ($utilizations as $utilization)
                                                <option value="{{ $utilization->id }}" {{ old('utilization_id', $asset->utilization_id) == $utilization->id ? 'selected' : '' }}>
                                                    {{ $utilization->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control @error('country_id') is-invalid @enderror" name="country_id" required>
                                            ...
                                            @foreach ($countries as $country)
                                                <option value="{{ $utilization->id }}" {{ old('country_id', $asset->country_id) == $country->id ? 'selected' : '' }}>
                                                    {{ $country->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control @error('language_id') is-invalid @enderror" name="language_id" required>
                                            ...
                                            @foreach ($languages as $language)
                                                <option value="{{ $language->id }}" {{ old('language_id', $asset->language_id) == $language->id ? 'selected' : '' }}>
                                                    {{ $language->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control @error('asset_type_id') is-invalid @enderror" id="asset_type_select" name="asset_type_id" required>
                                            ...
                                            @foreach ($assetTypes as $assetType)
                                                <option value="{{ $assetType->id }}" data-types="{{ $assetType->type }}" {{ old('asset_type_id', $asset->asset_type_id) == $assetType->id ? 'selected' : '' }}>
                                                    {{ $assetType->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        @if ($asset->file)
                                            <p>Current File: <a href="{{ asset('storage/' . $asset->file) }}" target="_blank">View</a></p>
                                        @endif
                                    </div>
                                </div>
                            </div>
